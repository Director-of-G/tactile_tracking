...
panel_center_and_offset = {
    "xfdistal": ([0, 0.0065, 0.016], -0.0135),
    "xfmiddle": ([0, 0.007, 0.0135], -0.016),
    "xfproximal": ([0, 0.008, 0.022], -0.017),
    "thdistal": ([0.009, 0, 0.018], -0.0185),
    "thmiddle": ([0.009, 0, 0.018], -0.018),
    "thproximal": ([0.011, 0, 0.018], -0.022)
}
taxel_spacing = 0.003
taxel_size = (4, 4)
taxel_radius = 0.001
...
tree = etree.parse("Adroit/resources/chain.xml")
root = tree.getroot()
# kin_tree and kin_root for pytorch_kinematics
kin_tree = etree.parse("Adroit/Adroit_hand_kin.xml")
kin_root = kin_tree.getroot()
...
def add_taxel_to_knuckle(knuckle: etree.Element, knuckle_kin: etree.Element, knuckle_key:str, offset_axis=1):
    ...
    if offset_axis == 0:
        ...
        mount_quat = np.array([-0.5, 0.5, -0.5, 0.5])
    elif offset_axis == 1:
        mount_pos = np.array(base_pos) + np.array([0, offset, 0])
        mount_quat = np.array([0.70710678, -0.70710678, 0., 0.])
...
tree.write('Adroit/resources/chain_with_touch.xml', pretty_print=True, xml_declaration=False, encoding='utf-8')
kin_tree.write('Adroit/Adroit_hand_kin_v2.xml', pretty_print=True, xml_declaration=False, encoding='utf-8')
# sensor_tree.write('Adroit/resources/touch_sensor_array.xml', pretty_print=True, xml_declaration=False, encoding='utf-8')
